...
export default function CoursePage({ slug, name, ...pagedContentProps }: CoursePageProps) {
    ...
    return <>
        ...
        <PagedContent {...pagedContentProps} getPagePath={page => {
            if (page === 1) return { href: "/[course]", as: `/${encodeURIComponent(slug)}` };
            return {
                href: "/[course]/page/[page]",
                as: `/${encodeURIComponent(slug)}/page/${page}`,
            };
        }}>
            {notes => notes.map(note => <p className="first-of-type:mt-4 mb-4" key={note.id}>
                <Link href="/[course]/[slug]" as={`/${encodeURIComponent(slug)}/${encodeURIComponent(note.slug)}`} className="link">
