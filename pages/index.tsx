...
export default function ClassNotes({ courses }: ClassNotesProps) {
    ...
    return <>
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {courses.map(course => {
                return <Link href="/[course]" as={`/${encodeURIComponent(course.slug)}`} className="block rounded-xl px-4 py-4 group hover:opacity-80 active:opacity-60 transition-opacity bg-ocean-100 dark:bg-ocean-900">
                    {course.name}
